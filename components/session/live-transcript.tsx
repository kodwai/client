...
function parseEventContent(event: SessionEvent): {
  ...
  // assistant_message — assistant text emitted by the hook after extracting from JSONL
  if (event.type === "assistant_message" || event.type === "assistant") {
    const text = (d.text as string) || (d.assistant_text as string) || "";
    return {
      label: "Assistant",
      variant: "assistant",
      title: "",
      body: text || "Claude responded",
    };
  }

  // Stop — Claude finished responding; show extracted assistant text if available
  // (kept as a fallback for older sessions; new sessions emit assistant_message instead)
  if (event.type === "Stop") {
    const assistantText = (d.assistant_text as string) || "";
    if (assistantText) {
      return {
        label: "Assistant",
        variant: "assistant",
