...
function fireConfetti(data: CelebrationData) {
  ...
  if (data.score < 50 && !data.personal_best && data.new_badges.length === 0 && !data.streak_milestone) return;
  const particleCount = data.score >= 80 ? 160 : data.score >= 50 ? 90 : 40;
  confetti({ particleCount, spread: 75, origin: { y: 0.3 }, scalar: 0.9 });
  if (data.personal_best || data.new_badges.length > 0 || data.streak_milestone) {
...
export function Celebration({ data }: { data: CelebrationData }) {
  ...
  if (
    ...
    !data.personal_best &&
    !(typeof data.streak === "number" && data.streak >= 2)
  )
    return null;
  ...
  return (
    <div className="my-4 flex flex-col gap-2" role="status" aria-live="polite">
      ...
      {typeof data.streak === "number" && data.streak >= 2 && (
        <div className="font-mono text-sm uppercase tracking-widest text-rust">🔥 {data.streak}-day streak</div>
      )}
